...
while (1) {
    ...
    } else if (DAC_RESOLUTION == 9) {
        for (i = 0; i < 512; i++) {
            dac.setVoltage(pgm_read_word(&(DACLookup_FullSine_9Bit[i])), false);
        }
    } else {
        // Use 8-bit data if nothing else is specified
        for (i = 0; i < 256; i++) {
